...
plot(lai_comparison[,3] + (1/12)*lai_comparison[,4], lai_comparison[,1], col = 'black', ylim = c(-1.5,3))
points(lai_comparison[,3] + (1/12)*lai_comparison[,4], lai_comparison[,2], col = 'red')
lines(lai_comparison[,3] + (1/12)*lai_comparison[,4], lai_comparison[,5], col = 'green')
...
plot(duke_observed$Year + (1/12)*duke_observed$Month, duke_observed[,6], col = 'black')
points(mean_monthly$Year + (1/12)*mean_monthly$Month, mean_monthly_normalized[,6], col = 'red')
...
load("/Users/laurapuckett/Documents/Research/Current/DAPPER/chains/Duke_for_EnKF_1.1.2018-03-30.15.11.15.Rdata")
...
# All Months ground obs
load('/Users/laurapuckett/Documents/Research/Current/DAPPER/chains/LAI_all_months.1.2018-05-06.18.10.18.Rdata')
...
# Winter Only ground obs
load("/Users/laurapuckett/Documents/Research/Current/DAPPER/chains/LAI_only_winter.1.2018-05-06.18.10.18.Rdata")
...
load("/Users/laurapuckett/Documents/Research/Current/DAPPER/chains/LAI_only_LANDSAT_normalized.1.2018-05-06.18.56.18.Rdata")
...
psi = 0.15 # uncertainty in lai observations
...
duke_realdata = duke_temp_realdata[which(duke_realdata$LAI != "NA" & duke_realdata$LAI != -99 & duke_realdata$Age != -99), ]
obs_used = "winter"
...
EnKF_DAPPER(psi, init_Fr, duke_observed, Fr_uncert, LAI_uncert, run_name = run_name)
output_DUKE <-   readRDS(paste(EnKF_directory, "/x_", run_name, ".RDS", sep = ""))
t_end = nrow(output_DUKE)
range_fr = range(output_DUKE[t_end,,7])
par(mfrow = c(1,1))
plot(density(output_DUKE[t_end, , 7]), col = "red", xlim = c(range_fr), ylim = c(0,10))
...
title = paste("Fr_uncert = ", Fr_uncert, ", psi = ", psi^2, ", obs = ", obs_used, ", LAI_uncert = ", LAI_uncert, sep = "")
print(title)
EnKF_DAPPER(psi, init_Fr, observed = mean_monthly, Fr_uncert, LAI_uncert, run_name =  run_name)
output_LANDSAT_1 <-   readRDS(paste(EnKF_directory, "/x_", run_name, ".RDS", sep = ""))

### LANDSAT trial #2 (without April and normalized)
...
title = paste("Fr_uncert = ", Fr_uncert, ", psi = ", psi^2, ", obs = ", obs_used, ", LAI_uncert = ", LAI_uncert, sep = "")
print(title)
EnKF_DAPPER(psi, init_Fr, observed = mean_monthly_normalized, Fr_uncert, LAI_uncert, run_name =  run_name)
output_LANDSAT_2 <-   readRDS(paste(EnKF_directory, "/x_", run_name, ".RDS", sep = ""))

### Comparison of output with LANDSAT and ground obs
par(mfrow = c(1,1))
quant_DUKE = array(NA, dim = c(nrow(output_DUKE), 3))
for (n in 1:nrow(output_DUKE)){
  quant_DUKE[n,] = quantile(output_DUKE[ n,, 1],c(0.10, 0.5, 0.90))
}

quant_LANDSAT_1 = array(NA, dim = c(nrow(output_LANDSAT_1), 3))
for (n in 1:nrow(output_LANDSAT_1)){
  quant_LANDSAT_1[n,] = quantile(output_LANDSAT_1[ n,, 1],c(0.10, 0.5, 0.90))
}

quant_LANDSAT_2 = array(NA, dim = c(nrow(output_LANDSAT_2), 3))
for (n in 1:nrow(output_LANDSAT_2)){
  quant_LANDSAT_2[n,] = quantile(output_LANDSAT_2[ n,, 1],c(0.10, 0.5, 0.90))
}

plot(1998 + (1/12)*1:nrow(output_DUKE), quant_DUKE[ ,1], type = 'l', col = 'orange', ylim = c(0,4))
lines(1998 + (1/12)*1:nrow(output_DUKE), quant_DUKE[ ,2], col = 'orange')
lines(1998 + (1/12)*1:nrow(output_DUKE), quant_DUKE[ ,3], col = 'orange')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,1], col = 'blue')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,2], col = 'blue')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,3], col = 'blue')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,1], col = 'purple')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,2], col = 'purple')
lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,3], col = 'purple')
points(mean_monthly$Year + (1/12)*mean_monthly$Month, mean_monthly$LAI, col = 'red')
points(duke_observed$Year + (1/12)*duke_observed$Month, duke_observed$LAI, col = 'green')
  legend(x = "top",inset = 0,
       legend = c("DUKE", "LANDSAT_2", 'DUKE', 'LANDSAT_2'),     col=c("blue","orange",'green','red'), lwd=5, cex= 0.75, horiz = TRUE)

  plot(1996 + (1/12)*1:nrow(output_DUKE),output_DUKE[ , 1, 1],type='l',ylim=c(0,5), main = 'DUKE', col = 'blue')
  for (n in 2:nmembers)
  {
    points(1996 + (1/12)*1:nrow(output_DUKE),output_DUKE[ , n, 1],type='l', col = 'blue') 
  }
  points(duke_observed$Year + (1/12)*duke_observed$Month, duke_observed$LAI, col = 'green')
  
  plot(1996 + (1/12)*1:nrow(output_LANDSAT_2), output_LANDSAT_2[ , 1, 1],type='l', col = 'orange', main = 'LANDSAT_2',  ylim = c(0,5))
  for (n in 1:nmembers)
  {
    points(1996 + (1/12)*1:nrow(output_LANDSAT_2), output_LANDSAT_2[ , n, 1],type='l', col = 'orange') 
  }
  points(mean_monthly$Year + (1/12)*mean_monthly$Month, mean_monthly$LAI, col = 'red')
  
plot(density(output_DUKE[nrow(output_DUKE), , 7]), col = 'blue', ylim = c(0,12), xlim = c(0.5,1.1))
lines(density(output_LANDSAT_1[nrow(output_LANDSAT_1), , 7]), col = 'orange', xlim = c(range_fr), ylim = c(0,10))
lines(density(fr_dist_from_mcmc), col = 'black')
lines(density(output_LANDSAT_2[nrow(output_LANDSAT_2), , 7]), col = 'cyan', ylim = c(0,12), xlim = c(0.5,1.1))
lines(density(mcmc_lai_only_winter_params[1000:1500,78]), col = 'red')
lines(density(mcmc_lai_only_winter_LANDSAT_normalized_params[1000:1500,78]), col = 'purple')

legend(x = "top",inset = 0,
     legend = c('MCMC all data','MCMC winter ground',"EnKF winter ground",' MCMC LANDSAT normalized','ENKF LANDSAT normalized',"EnKF LANDSAT"),     col=c('black','red',"blue",'purple','cyan',"orange"), lwd=5, cex= 0.75, horiz = FALSE)
