...
### OUTDATED PLOTS - FIX LEGENDS IF USING
# par(mfrow = c(1,1))
# quant_field_winter = array(NA, dim = c(nrow(output_field_winter), 3))
# for (n in 1:nrow(output_field_winter)){
#   quant_field_winter[n,] = quantile(output_field_winter[ n,, 1],c(0.10, 0.5, 0.90))
# }
# 
# par(mfrow = c(1,1))
...
# }
# 
# plot(1998 + (1/12)*1:nrow(output_field_winter), quant_field_winter[ ,1], type = 'l', col = 'orange', ylim = c(0,4))
# lines(1998 + (1/12)*1:nrow(output_field_winter), quant_field_winter[ ,2], col = 'orange')
# lines(1998 + (1/12)*1:nrow(output_field_winter), quant_field_winter[ ,3], col = 'orange')
# plot(1998 + (1/12)*1:nrow(quant_field_all_months), quant_field_all_months[ ,1], type = 'l', col = 'purple', ylim = c(0,8))
# lines(1998 + (1/12)*1:nrow(quant_field_all_months), quant_field_all_months[ ,2], col = 'purple')
# lines(1998 + (1/12)*1:nrow(quant_field_all_months), quant_field_all_months[ ,3], col = 'purple')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,1], col = 'blue')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,2], col = 'green')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_1), quant_LANDSAT_2[ ,3], col = 'red')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,1], col = 'purple')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,2], col = 'orange')
# lines(1998 + (1/12)*1:nrow(output_LANDSAT_2), quant_LANDSAT_2[ ,3], col = 'pink')
...
#   legend(x = "top",inset = 0,
#        legend = c("DUKE", 'ENKF field all months',"LANDSAT_2", 'DUKE', 'LANDSAT_2'),     col=c("blue","orange",'green','red'), lwd=5, cex= 0.75, horiz = FALSE)
# 
#   plot(1996 + (1/12)*1:nrow(output_field_winter),output_field_winter[ , 1, 1],type='l',ylim=c(0,5), main = 'ENKF field winter', col = 'blue')
#   for (n in 2:nmembers)
#   {
#     points(1996 + (1/12)*1:nrow(output_field_winter),output_field_winter[ , n, 1],type='l', col = 'blue') 
#   }
#   points(duke_observed$Year + (1/12)*duke_observed$Month, duke_observed$LAI, col = 'green')
#   
...
lines(density(output_field_winter[nrow(output_field_winter) - 8, , 7]), col = 'blue',lty=2)
#lines(density(output_field_winter[, , 7]), col = 'green',lty=2)


lines(density(output_LANDSAT_1[nrow(output_LANDSAT_1), , 7]), col = 'green', lty=2)
...
legend(x = "topleft",inset = 0,
     legend = c('MCMC all data','Field Measurements, Winter','LANDSAT', 'LANDSAT normalized'),     col=c('black','blue','red','green'), lty = c(1,3,3,3), lwd=3, cex= 0.7, horiz = FALSE)
#legend(x = "top",inset = 0,
